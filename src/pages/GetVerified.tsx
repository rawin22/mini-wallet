...
export const GetVerified: React.FC = () => {
    ...
    const [attachments, setAttachments] = useState<FileAttachmentInfoItem[]>([]);
    ...
    // ── Reset: delete attachments and restart wizard ──
    const handleResetVerification = async () => {
        ...
        try {
            // Delete all existing file attachments for this customer
            for (const att of attachments) {
                const attId = att.FileAttachmentId ?? att.fileAttachmentId;
                if (attId) {
                    try {
                        await verificationService.deleteFileAttachment(attId);
                        console.log('[Reset] Deleted attachment:', attId);
                    } catch (err) {
                        console.warn('[Reset] Failed to delete attachment (continuing):', attId, err);
                    }
                }
            }
    ...
    return (
        <div className="get-verified-page">
            ...
            {/* Reset option when resuming mid-wizard */}
            {step > 1 && step < 4 && !showResetConfirm && (
                <div className="gv-restart-link">
                    <button
            ...
            {showResetConfirm && step < 4 && (
                <div className="gv-reset-warning gv-card">
                    <p className="gv-reset-warning-text">{t('verification.resetWarning')}</p>
                    <div className="gv-reset-actions">
                        <button
                            className="primary-btn gv-reset-confirm-btn"
                            onClick={handleResetVerification}
                            disabled={processing}
                        >
                            {processing ? t('verification.resetting') : t('verification.confirmReset')}
                        </button>
                        <button
            ...
            {step === 4 && (
                <div className="gv-card gv-confirmation">
                    ...
                    <div className="gv-reset-section">
                        {!showResetConfirm ? (
                            <button
                                className="secondary-btn gv-reset-btn"
                                onClick={() => setShowResetConfirm(true)}
                                disabled={processing}
                            >
                                {t('verification.startNewVerification')}
                            </button>
                        ) : (
                            <div className="gv-reset-warning">
                                <p className="gv-reset-warning-text">{t('verification.resetWarning')}</p>
                                <div className="gv-reset-actions">
                                    <button
                                        className="primary-btn gv-reset-confirm-btn"
