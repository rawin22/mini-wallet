...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    const init = async () => {
      const shouldForceAuthReset = import.meta.env.VITE_FORCE_AUTH_RESET === 'true';
      const hasDoneForcedReset =
        localStorage.getItem(API_CONFIG.STORAGE_KEYS.AUTH_RESET_DONE) === 'true';

      if (shouldForceAuthReset && !hasDoneForcedReset) {
        storage.clearAuth();
        localStorage.setItem(API_CONFIG.STORAGE_KEYS.AUTH_RESET_DONE, 'true');
      }
